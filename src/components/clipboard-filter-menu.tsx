...
export const ClipboardFilterMenu = () => {
	...
	const hasActiveFilters =
		filters.favorite || filters.contentTypes.size > 0 || filters.dateRange !== 'all';

	const activeFilterCount =
		(filters.favorite ? 1 : 0) + filters.contentTypes.size + (filters.dateRange !== 'all' ? 1 : 0);
	...
	return (
		<DropdownMenu>
			...
			>
				...
				<FunnelPlus className={cn('size-4 hidden', hasActiveFilters && 'block')} />
				{activeFilterCount > 0 && (
			...
			<DropdownMenuContent align="end" sideOffset={6} className="w-52">
				<DropdownMenuGroup>
					<DropdownMenuCheckboxItem checked={filters.favorite} onCheckedChange={toggleFavorite}>
				...
				<DropdownMenuGroup>
					...
					{(Object.keys(CONTENT_FILTER_LABELS) as ContentFilter[]).map((type) => (
						<DropdownMenuCheckboxItem
							key={type}
